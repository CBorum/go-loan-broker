...
func TestJsonInput(t *testing.T) {
	...
	q, err := bankutil.StdQueueDeclare(ch, "ckkm-xml-out")
	...
	bankutil.Publish(ch, body, "", q.Name)
	bankutil.Publish(ch, body2, "", q.Name)
	bankutil.Publish(ch, body3, "", q.Name)
	bankutil.Publish(ch, body4, "", q.Name)
	bankutil.Publish(ch, body5, "", q.Name)
...
func TestJsonInputCPH(t *testing.T) {
	log.SetFlags(log.LstdFlags | log.Lshortfile | log.Ltime)
	conn, err := amqp.Dial(bankutil.RabbitURL)
	bankutil.FailOnError(err, "Failed to connect to RabbitMQ")
	defer conn.Close()
	...
	q, err := bankutil.StdQueueDeclare(ch, "ckkm-xml-out") //doesn't matter which
	bankutil.FailOnError(err, "Failed to declare queue")

	lr := bankutil.CPHLoanResponse{
		InterestRate: 4.5,
		Ssn:          234123412345,
	}
	lr2 := bankutil.CPHLoanResponse{3.2, 234223412345, "bank1"}
	lr3 := bankutil.CPHLoanResponse{5.5, 123412345, "bank2"}
	lr4 := bankutil.CPHLoanResponse{5.5, 234123412345, "bank3"}
	lr5 := bankutil.CPHLoanResponse{4.5, 234123412345, "bank4"}
	body, _ := json.Marshal(lr)
	body2, _ := json.Marshal(lr2)
	body3, _ := json.Marshal(lr3)
	body4, _ := json.Marshal(lr4)
